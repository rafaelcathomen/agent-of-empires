...
By default (`session.smart_rename_timing = "turn_end"`) the rename waits for the first turn to finish and titles from the whole transcript, your prompt and the agent's response, so the title reflects what the turn did. Set it to `"prompt_start"` to title immediately from your first prompt alone, so the sidebar updates without waiting for the turn; the one-shot then races the live agent for the provider API, which is why it is opt-in.
